...
export function LoginPage() {
  ...
  const submit = async (data: Data) => {
    try {
      ...
      setError('Não conseguimos realizar o login agora. Verifique seus dados e tente novamente.')
    }
  }
  ...
  return <AuthShell title="Boas-vindas" subtitle="Entre para acompanhar sua vida financeira." darkForm>
    ...
    <form onSubmit={handleSubmit(submit)} className="mt-8 space-y-5">
      <div>
        ...
        <input className="field" autoComplete="email" placeholder="voce@email.com" {...register('email')} />
        {errors.email && <p className="mt-1 text-xs text-red-300">{errors.email.message}</p>}
      </div>
      <div>
        <div className="flex justify-between">
          <label className="label">Senha</label>
          <Link className="text-xs font-medium text-cyan-300 hover:text-cyan-200" to="/esqueci-senha">Esqueci minha senha</Link>
        </div>
      ...
      <label className="flex items-center gap-2 text-sm text-slate-300">
        <input type="checkbox" className="rounded border-slate-300 text-blue-600" {...register('keep')} />
        Manter conectado
      </label>
      {error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      <Button className="w-full" disabled={isSubmitting}>{isSubmitting ? 'Entrando...' : 'Entrar'}</Button>
    </form>
    <p className="mt-7 text-center text-sm text-slate-300">Ainda não tem conta? <Link className="font-semibold text-cyan-300 hover:text-cyan-200" to="/cadastro">Criar conta</Link></p>
  </AuthShell>
}
