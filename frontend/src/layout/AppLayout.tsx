...
type MenuItem = readonly [path: string, label: string, icon: typeof LayoutDashboard]
type MenuGroup = { id: GroupId; label: string; icon: typeof LayoutDashboard; items: readonly MenuItem[] }
...
function groupForPath(pathname: string): GroupId | null {
  return groups.find((group) => group.items.some(([path]) => pathname.startsWith(path)))?.id ?? null
}
...
export function AppLayout() {
  ...
  useEffect(() => {
    const closePanels = (event: MouseEvent) => {
      if (!headerRef.current?.contains(event.target as Node)) setHeaderPanel(null)
    }
        ...
        {(user?.is_assistant ? groups.slice(0, 1) : groups).map((group) => {
          ...
          return <div key={group.id}>
            ...
            <div className={cn('grid transition-all duration-200', expanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0')}>
              <div className="overflow-hidden py-1">{group.items.map((item) => navItem(item, true))}</div>
            </div>
          </div>
        })}
  ...
  return <div className="min-h-screen bg-[var(--app-bg)]">
    ...
    {criticalToast && <div className="fixed inset-x-3 bottom-3 z-[70] rounded-xl bg-red-600 px-4 py-3 text-sm font-medium text-white shadow-xl sm:inset-x-auto sm:bottom-5 sm:right-5">Um novo alerta crítico precisa da sua atenção.</div>}

    <div className="hidden h-screen md:fixed md:inset-y-0 md:flex">{side}</div>
    {mobileMenuOpen && <div className="fixed inset-0 z-40 bg-slate-900/30 md:hidden" onClick={() => setMobileMenuOpen(false)} />}
    <div className={cn('fixed inset-y-0 left-0 z-50 transition-transform md:hidden', mobileMenuOpen ? 'translate-x-0' : '-translate-x-full')}>
      {side}
      <button aria-label="Fechar menu" className="absolute right-[-2.5rem] top-3 rounded bg-white p-2" onClick={() => setMobileMenuOpen(false)}><X size={18} /></button>
    </div>
      ...
      <header className="sticky top-0 z-30 flex h-16 items-center border-b bg-white/95 px-5 shadow-sm backdrop-blur">
        ...
        <div ref={headerRef} className="relative ml-auto flex items-center gap-2">
          <div className="relative">
            ...
            >
              <Bell size={20} />
              <span className="absolute -right-1 -top-1"><AlertBadge count={alerts.data?.unread ?? 0} /></span>
            </button>
            {headerPanel === 'notifications' && <div className="absolute right-0 top-12 w-[min(22rem,calc(100vw-2rem))] rounded-xl border bg-white p-3 shadow-xl">
              <div className="flex items-center justify-between border-b px-1 pb-3">
                ...
                <span className="text-xs text-slate-500">{alerts.data?.unread ?? 0} não lidas</span>
              </div>
              <div className="max-h-72 overflow-y-auto py-2">
                {alerts.data?.highlights.length ? alerts.data.highlights.slice(0, 4).map((alert) => <NavLink
                  key={alert.id}
                ...
                >
                  ...
                  <p className="mt-1 line-clamp-2 text-xs text-slate-500">{alert.descricao}</p>
                </NavLink>) : <p className="px-3 py-8 text-center text-sm text-slate-400">Nenhuma notificação no momento.</p>}
              </div>
              <NavLink to="/alertas" onClick={() => setHeaderPanel(null)} className="block border-t pt-3 text-center text-sm font-semibold text-blue-600">Ver todas as notificações</NavLink>
            </div>}
          </div>
          ...
          <div className="relative">
            ...
            {headerPanel === 'account' && <div className="absolute right-0 top-12 w-[min(20rem,calc(100vw-1rem))] rounded-xl border bg-white p-4 shadow-xl">
              <div className="flex items-center gap-3 border-b pb-4">
                <span className="grid h-11 w-11 place-items-center rounded-full bg-blue-100 font-bold text-blue-700">{user?.name.slice(0, 2).toUpperCase()}</span>
                <div className="min-w-0"><p className="truncate font-semibold text-slate-900">{user?.name}</p><p className="truncate text-xs text-slate-500">{user?.email}</p></div>
              </div>
              <dl className="space-y-3 py-4 text-sm">
                <div className="flex gap-3"><Stethoscope size={17} className="mt-0.5 text-slate-400" /><div><dt className="text-xs text-slate-400">CRM</dt><dd className="text-slate-700">{user?.crm} / {user?.crm_uf}</dd></div></div>
                <div><dt className="text-xs text-slate-400">Especialidade</dt><dd className="text-slate-700">{user?.specialty}</dd></div>
                <div><dt className="text-xs text-slate-400">Localização</dt><dd className="text-slate-700">{user?.city} / {user?.state}</dd></div>
              </dl>
              <NavLink to="/perfil" onClick={() => setHeaderPanel(null)} className="block rounded-lg bg-blue-50 px-3 py-2 text-center text-sm font-semibold text-blue-700">Ver informações da conta</NavLink>
              <button onClick={() => void logout()} className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-slate-50"><LogOut size={16} />Sair</button>
            </div>}
          </div>
        </div>
      </header>
      <div className="min-w-0 p-3 min-[380px]:p-4 sm:p-6 lg:p-8"><Outlet /></div>
    </main>
  </div>
}
