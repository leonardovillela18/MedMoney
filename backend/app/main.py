...
app=FastAPI(title='MedMoney API',version=settings.app_version,docs_url='/docs' if settings.enable_docs else None,redoc_url='/redoc' if settings.enable_docs else None,openapi_url='/openapi.json' if settings.enable_docs else None)
...
for router in (auth_router,contractors_router,shifts_router,receivables_router,dashboard_router,invoices_router,taxes_router,cashflow_router,expenses_router,today_router,insights_router,analytics_router,goals_router,alerts_router):app.include_router(router,prefix='/api/v1')
