...
def export(format:str=Query('csv',pattern='^(csv|xlsx|pdf|svg)$'),f:dict=Depends(filters),user:User=Depends(current_user),db:Session=Depends(get_db)):
 ...
 if format=='csv':
  output=io.StringIO();writer=csv.writer(output);writer.writerows(rows);return Response('\ufeff'+output.getvalue(),media_type='text/csv',headers={'Content-Disposition':'attachment; filename=crmoney-analytics.csv'})
 if format=='xlsx':
  from openpyxl import Workbook
  book=Workbook();sheet=book.active;sheet.title='Resumo Executivo'
  for row in rows:sheet.append(row)
  sheet.freeze_panes='A2';sheet.auto_filter.ref=sheet.dimensions;binary=io.BytesIO();book.save(binary);binary.seek(0);return StreamingResponse(binary,media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',headers={'Content-Disposition':'attachment; filename=crmoney-analytics.xlsx'})
 if format=='pdf':
  from reportlab.lib.pagesizes import A4
  from reportlab.pdfgen import canvas
  binary=io.BytesIO();pdf=canvas.Canvas(binary,pagesize=A4);pdf.setTitle('Relatório Executivo CRMoney');pdf.setFont('Helvetica-Bold',18);pdf.drawString(45,800,'CRMoney — Relatório Executivo');pdf.setFont('Helvetica',9);pdf.drawString(45,782,f'Gerado em {date.today().strftime("%d/%m/%Y")}');y=750
  ...
  pdf.save();binary.seek(0);return StreamingResponse(binary,media_type='application/pdf',headers={'Content-Disposition':'attachment; filename=crmoney-relatorio.pdf'})
 values=[float(x['value']) for x in data['revenue']['monthly']];maximum=max(values+[1]);bars=''.join(f'<rect x="{30+i*24}" y="{180-v/maximum*140}" width="16" height="{v/maximum*140}" rx="3" fill="#2563eb"/>' for i,v in enumerate(values[-20:]));svg=f'<svg xmlns="http://www.w3.org/2000/svg" width="560" height="220"><rect width="100%" height="100%" fill="white"/><text x="20" y="24" font-family="Arial" font-size="16">CRMoney — Receita mensal</text>{bars}</svg>';return Response(svg,media_type='image/svg+xml',headers={'Content-Disposition':'attachment; filename=crmoney-grafico.svg'})
