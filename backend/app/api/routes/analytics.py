...
def export(format:str=Query('csv',pattern='^(csv|xlsx|pdf|svg)$'),f:dict=Depends(filters),user:User=Depends(current_user),db:Session=Depends(get_db)):
 ...
 if format=='pdf':
  ...
  binary=io.BytesIO();pdf=canvas.Canvas(binary,pagesize=A4);pdf.setTitle('Relatório Executivo MedFinance');pdf.setFont('Helvetica-Bold',18);pdf.drawString(45,800,'MedFinance — Relatório Executivo');pdf.setFont('Helvetica',9);pdf.drawString(45,782,f'Gerado em {date.today().strftime("%d/%m/%Y")}');y=750
 ...
 values=[float(x['value']) for x in data['revenue']['monthly']];maximum=max(values+[1]);bars=''.join(f'<rect x="{30+i*24}" y="{180-v/maximum*140}" width="16" height="{v/maximum*140}" rx="3" fill="#2563eb"/>' for i,v in enumerate(values[-20:]));svg=f'<svg xmlns="http://www.w3.org/2000/svg" width="560" height="220"><rect width="100%" height="100%" fill="white"/><text x="20" y="24" font-family="Arial" font-size="16">MedFinance — Receita mensal</text>{bars}</svg>';return Response(svg,media_type='image/svg+xml',headers={'Content-Disposition':'attachment; filename=medmoney-grafico.svg'})
