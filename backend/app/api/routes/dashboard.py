...
def dashboard(user:User=Depends(current_user),db:Session=Depends(get_db)):
 today=date.today();month=today.replace(day=1);next_month=month.replace(year=month.year+(month.month==12),month=1 if month.month==12 else month.month+1)
 ...
 pending=total(Receivable.remaining_balance,[Receivable.user_id==user.id,Receivable.deleted_at.is_(None)]);received=total(Receivable.received_value,[Receivable.user_id==user.id,Receivable.received_date>=month,Receivable.received_date<next_month,Receivable.deleted_at.is_(None)]);overdue=total(Receivable.remaining_balance,[Receivable.user_id==user.id,Receivable.expected_date<today,Receivable.status!='Recebido',Receivable.deleted_at.is_(None)]);hours=total(Shift.duration_hours,[Shift.user_id==user.id,Shift.date>=month,Shift.date<next_month,Shift.deleted_at.is_(None)]);cash=CashflowService(db).projection(user.id)['summary']
 summary={'total_expected':pending,'received_month':received,'pending':pending,'overdue':overdue,'estimated_tax':cash['tax_reserve'],'estimated_profit':cash['available'],'shifts_month':len([s for s in shifts if month<=s.date<next_month]),'hours':hours,**cash}
