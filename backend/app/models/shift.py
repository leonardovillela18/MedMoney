...
class Shift(Base):
    ...
    id:Mapped[uuid.UUID]=mapped_column(primary_key=True,default=uuid.uuid4); user_id:Mapped[uuid.UUID]=mapped_column(ForeignKey('users.id'),index=True); contractor_id:Mapped[uuid.UUID]=mapped_column(ForeignKey('contractors.id'),index=True);specialty_id:Mapped[uuid.UUID|None]=mapped_column(ForeignKey('medical_specialties.id'),index=True);city_ibge_code:Mapped[str|None]=mapped_column(String(7),index=True)
    tax_reserve_percentage:Mapped[Decimal|None]=mapped_column(Numeric(6,3));tax_treatment:Mapped[str]=mapped_column(String(20),default='PJ_TAXABLE',index=True)
